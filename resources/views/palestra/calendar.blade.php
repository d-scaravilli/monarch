            ...
            <div class="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
                <div class="flex items-center gap-3">
                    <a href="{{ route('palestra.calendar', array_filter(['view' => $view, 'date' => $prevAnchor->toDateString()] + request()->only('course_id', 'room_id'))) }}"
                       class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-gray-500 hover:bg-gray-100 dark:hover:bg-white/5">
                        <x-heroicon-o-chevron-right class="h-4 w-4 rotate-180" />
                    </a>
                    <span class="font-semibold text-gray-900 dark:text-gray-100 capitalize text-center flex-1 sm:flex-none sm:w-48">{{ $label }}</span>
                    ...
                    <a href="{{ route('palestra.calendar', request()->only('course_id', 'room_id')) }}" class="shrink-0 text-sm font-medium text-gray-500 hover:text-gray-700 dark:hover:text-gray-300">
                        Oggi
                    </a>
                </div>
